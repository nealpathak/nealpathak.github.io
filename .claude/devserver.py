...
This exists because of a real incident: the stock server lets a browser reuse
cached ES modules across reloads, and a day's verification silently ran against
the *previous* day's code. Everything looked fine, and the measurements were
meaningless. Caching is exactly wrong for a dev loop where the whole point is to
check that the change you just made behaves.
"""
